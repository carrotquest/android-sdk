...
class InputDataPresenter(var view: IInputDataView?) {
    ...
    fun onTryConnect(context: Context, apiKey: String, userAuthKey: String) {
        view?.showProgress()
        if (apiKey.isNotEmpty() && userAuthKey.isNotEmpty()) {
            Dashly.setup(context, apiKey, object : CarrotSDK.Callback<Boolean> {
                ...
                override fun onResponse(resConnect: Boolean) {
                    if (resConnect) {
                        saveData(context, apiKey, userAuthKey)
    ...
    private fun saveData(context: Context, apiKey: String, userAuthKey: String) {
